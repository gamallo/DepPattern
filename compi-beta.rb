...
def name_function
	...
	file_nf = File.open("src/dependencies.conf")
...
def initial_code
	...
	file_ts.each do |line|
		if line[0..0] != "#" && line[0..-1] != "" && limpa_brancos(line.split(" ")[1]) != nil
			...
			out << '$' + limpa_brancos(line.split(" ")[0]) + ' = "' + exp + '";' + "\n"
	...
	# lexical_classes.conf
 	file_lc = File.open("src/lexical_classes.conf")
	file_lc.each do |line|
		if line[0..0] != "#" && line[0..-1] != "" && limpa_brancos(line.split("=")[1]) != nil
			out << line.split("=")[0] + ' = "(?:' + limpa_brancos(line.split("=")[1]).gsub(" ",'\\\\|') + '\\|)";' + "\n"
		end
	end
	out
end
...
class Definition
	...
	def pre_code
		...
		if @negative_context_l != ""
			out_code << "my $CNTXL = negL($#{@negative_context_l});\n"
		end
		if @negative_context_r != ""
			out_code << "my $CNTXR = negR($#{@negative_context_r});\n"
		end
		if out_code == ""
			return nil
		end
	...
	def main_code
		...
		if $name_function[@name] == $name_function.default
			raise StandardError, "Warning. Function for the rule '" + @name + "' was not found. Try to add the function to the file src/dependencies.src"
		end
		out_code
	end
...
#print "\n"
		...
		a1 = a1.gsub("&",'\|${b}')
                ...
                #a1 = a1.gsub('|(?:$CNTXL$a)?', '|')  ###CAMBIO DE PABLO: parche contextos negativos optativos (nao tenhem sentido)
                a1 = a1.gsub('|(?:$CNTXL$a)', '|')  ###CAMBIO DE PABLO: parche contextos negativos
                ...
                a1 = a1.gsub("$a$a","$a") # Corrige o duplo $a que poderia produzir-se com os super-atalhos
				...
				if ($rel_position[$name_function[@name]] == no_context) # Case of no_context and Rel position are equals
					...
					if @agr != "" # Needed because in blocks this function is called and it can exist a rule in block with no agreement
						if @before_regexp_subs != nil
							...
							nova << pedacos_before[index].gsub("${l}","${l}concord:1${b}").gsub("$a","${l}concord:1${r}") ##CAMBIO PABLO: o concord vai apos $l
						else
							nova << pedaco.gsub("${l}","${l}concord:1${b}").gsub("$a","${l}concord:1${r}") ##CAMBIO PABLO: o concord vai apos $l
...
if ARGV.size < 1
	puts 'Error when calling compiler. CORRECT SINTAX: "ruby compiXX.rb gramma.txt" [name_parser]'
else
	# Execute pre-compi a script that makes previous modifications
	if File.file?("scripts/pre-compi.perl") and system "./scripts/pre-compi.perl #{ARGV[0]} > log/grammar_not_compiled"
		file_in_name = "log/grammar_not_compiled"
	else
		file_in_name = ARGV[0]
	end
	# Change name
	if ARGV[1] != nil
		NAME_OUT_FILE = ARGV[1] + ".perl"
	else
		NAME_OUT_FILE = $NAME_OUT_FILE
	end
	# Files operations
	file_in = File.open(file_in_name)
	if File.file?(NAME_OUT_FILE)
		system "rm #{NAME_OUT_FILE}"
	end
	file_out = File.new(NAME_OUT_FILE,"w") 
	file_pre = File.open("src/pre_parsingCascataByRegularExpressions.perl","r")
	file_pre.each { |line| file_out.write line.sub("#__ICODE__",initial_code) }
	file_pre.close
	...
	file_post = File.open("src/post_parsingCascataByRegularExpressions.perl","r")
	system "chmod +x #{NAME_OUT_FILE}"
